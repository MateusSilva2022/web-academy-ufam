...
- O script `bd/script.sql` é executado automaticamente.
- Os dados do MySQL são persistidos em um volume Docker.
- Os logs do backend são armazenados na pasta `backend/log`.

## Observação

Foi adicionado o comando abaixo ao Dockerfile do backend para gerar o Prisma Client antes da compilação da aplicação:

```dockerfile
RUN npx prisma generate
```

Essa alteração foi necessária para que o projeto pudesse ser compilado corretamente durante o build da imagem Docker.
